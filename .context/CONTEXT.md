...
-   The prompt is sent to the configured LLM provider (e.g., Gemini) via the `LLMProvider` interface.
-   The generated markdown content is validated (minimum length, header/section presence) and ensures a trailing newline.
...
-   The generated markdown content is validated.
...
3.  **`LLMError` (`src/context_hook/llm.py`)**: Base exception class for all LLM-related errors across providers.
4.  **`LLMProvider` (`src/context_hook/llm.py`)**: Abstract base class defining the `generate` method that all LLM provider implementations must adhere to. This allows for a pluggable LLM backend.
5.  **Git Objects (from `gitpython`)**: Internally used `Repo`, `Commit`, `Diff` objects to interact with the Git repository.
...
### External API:
*   **Configured LLM API (via `LLMProvider` interface)**: Accessed via provider-specific SDKs (e.g., `google-genai` for Gemini).
    *   `LLMProvider.generate(prompt: str, max_retries: int = 3) -> str`: Sends a text prompt to the configured LLM, returns generated text. Includes retry logic with exponential backoff for rate limit errors. Handles `LLMError` for API failures. Implementations are expected to strip common markdown code block wrappers from the beginning and end of the generated text.
...
*   **`context_hook.gemini`**:
    *   `GeminiClient(api_key: str, model: str)`: Initializes the Gemini client. This class now implements the `LLMProvider` abstract base class.
...
*   **`context_hook.llm`**:
    ...
    *   `LLMProvider`: Abstract base class defining the common interface for all LLM providers (e.g., `generate`).
    *   `get_provider(config: Config) -> LLMProvider`: Factory function that dynamically instantiates and returns the correct `LLMProvider` implementation based on the `config.provider` setting (e.g., `GeminiClient`).
...
*   `google-genai>=1.0.0`: Primary dependency for interacting with the Gemini API (used by `GeminiClient`).
...
*   `LLM_API_KEY`: An optional generic environment variable for the LLM API key. If set, it will be used for any configured provider.
...
*   **Design Principles**: Adheres to principles like "context is assistive, not authoritative," "Git hooks never block commits," "failures are graceful," and "all AI content is reviewable." The new `LLMProvider` interface enhances modularity and extensibility for future LLM integrations.
