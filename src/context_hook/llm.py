...
def get_provider(config) -> LLMProvider:
    ...
    if provider_name == "gemini":
        ...
        return GeminiClient(api_key=config.get_api_key(), model=config.model)
        
    # Future providers (e.g., openai, anthropic) would be added here
    # elif provider_name == "openai":
    #     from context_hook.openai import OpenAIClient
    #     return OpenAIClient(api_key=config.get_api_key(), model=config.model)
